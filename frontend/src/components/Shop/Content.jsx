...
const Content = () => {
    ...
    const options = ['Sort By Popularity', 'Sort By Price: Low To High', 'Sort By Price: High To Low'];
        ...
        const fetchCategoryCounts = async () => {
            try {
                ...
                const updatedCategoryCount = categoryCount.map((category, index) => {
                    if (index === 0) {
                        const sum = responses.reduce((accumulator, response) => accumulator + response.data.quantity, 0);
                        return {
                            ...category,
                            count: sum,
                        };
    ...
    const renderOptions = options.map((option, index) => (
        <div className="dropdown-option" key={index} onClick={() => handleOptionClick(option)}>
            {option}
        </div>
    ));
    ...
    const dropdownContent = isOpen && <div className="dropdown-content">{renderOptions}</div>;
                                    ...
                                    <div className='category'>
                                        ...
                                        <div className='categories'>
                                            {categoryCount.map((category, index) => (
                                                <div className='category-item' key={index} onClick={() => setSelectedCategory(category.name)}>
